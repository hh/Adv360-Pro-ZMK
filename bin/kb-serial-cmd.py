...
devs = sorted(glob.glob("/dev/serial/by-id/usb-Kinesis_Corporation_Adv360_Pro_*"))
devs = [d for d in devs if ("_rt_" in d) == (side == "right")]
if not devs:
    sys.exit(f"no Adv360 {side}-half serial console found (connected via USB?)")

# Firmware with the Studio snippet exposes two CDC-ACM interfaces per half
# (command console + Studio RPC) and their order isn't stable — send to
# all of them; the RPC port ignores stray text.
buf = b""
for dev in devs:
    try:
